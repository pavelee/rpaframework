...
export const Form = (props: {
  ...
  const components: JSX.Element[] = props.elements
    ...
    .map((component, index) => (
      <Box key={index} marginBottom="20px" textAlign="initial" width="100%">
        {component}
      </Box>
    ));

  // Append submit button if none defined
  if (!props.elements.find(Types.isSubmit)) {
    const hasInput = props.elements.some(Types.isInput);
    const element: Types.Submit = {
      type: 'submit',
      buttons: hasInput ? ['Submit'] : ['Close'],
    };
    components.push(
      <Submit key={components.length} element={element} onSubmit={onSubmit} />,
    );
  }
