...
export const Title = (props: { element: Types.Title }): JSX.Element => {
  let title: JSX.Element;
  switch (props.element.size) {
    case 'small':
      title = <H4>{props.element.value}</H4>;
      break;
    case 'medium':
      title = <H3>{props.element.value}</H3>;
      break;
    case 'large':
      title = <H2>{props.element.value}</H2>;
      break;
    default:
      unreachable(props.element.size);
  }
  return <Centered>{title}</Centered>;
};
